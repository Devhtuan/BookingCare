...
export default function BookingModal({
  ...
  const { isLoggedIn, userInfo, language, genders_admin } = useSelector(
    (state) => ({
  ...
  useEffect(() => {
    async function fetchData() {
      if (userInfo) {
        ...
        setBirthday(new Date(parseInt(user.birthday)));
        setSelectedGender(
          language === "vi"
            ? user.gender === "M"
              ? { label: "Nam", value: "M" }
  ...
  useEffect(() => {
    ...
    setGenders(buildDataGender(genders));
    setSelectedGender(
      language === "vi"
        ? user.gender === "M"
          ? { label: "Nam", value: "M" }
  ...
  const handleConfirmBooking = async () => {
    ...
    let clinicId_temp = clinicId
      ? clinicId
      : dataTime.doctorData.Doctor_Infor.clinicId
      ? dataTime.doctorData.Doctor_Infor.clinicId
      : null;
    ...
    if (res && res.errCode === 0) {
      ...
      if (language === "en") {
        toast.warn(
          "You have already booked an appointment at this time, please choose a different time slot!"
        );
      } else {
        toast.warn(
          "Bạn đã đặt lịch hẹn vào khung giờ này rồi, vui lòng chọn khung giờ khác!"
        );
      }
      closeBookingClose();
    } else {
